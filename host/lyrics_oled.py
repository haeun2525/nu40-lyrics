...
KIND_BARS, KIND_LYRIC = 0x01, 0x02
...
def play(levels: np.ndarray, beats: np.ndarray,
         cues: list[tuple[float, str]], audio: Path, args) -> None:
    ser = None
    if not args.preview:
        import serial
...
def main() -> None:
    ...
    p.add_argument("--lrc", required=True, help="가사 타이밍 파일(.lrc)")
    ...
    bad = lrclib.missing_glyphs(cues, lrclib.load_font_codes(FONT_HEADER))
    if bad:
    ...
    # 첫 가사가 나오기 전 빈 구간에 곡 정보를 띄운다.
    intro = intro_cue(meta, title)
    ...
    play(levels, beats, cues, audio, args)
